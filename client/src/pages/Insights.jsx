...
const MOOD_LABEL = {
  great:   { emoji: '😄', label: 'Great',   color: 'text-[var(--pa-accent-success)] bg-[var(--pa-accent-success-soft)]' },
  good:    { emoji: '🙂', label: 'Good',    color: 'text-[var(--pa-accent-blue)] bg-[var(--pa-accent-blue-soft)]' },
  okay:    { emoji: '😐', label: 'Okay',    color: 'text-[var(--pa-accent-warning)] bg-[var(--pa-accent-warning)]/10' },
  low:     { emoji: '😔', label: 'Low',     color: 'text-[var(--pa-accent-danger)]/80 bg-[var(--pa-accent-danger)]/10' },
  stressed:{ emoji: '😤', label: 'Stressed',color: 'text-[var(--pa-accent-danger)] bg-[var(--pa-accent-danger)]/10' },
};

const PRIORITY_CFG = {
  high:     { bg: 'bg-[var(--pa-accent-danger)]/10',    border: 'border-[var(--pa-accent-danger)]/30',    text: 'text-[var(--pa-accent-danger)]',    badge: 'High' },
  medium:   { bg: 'bg-[var(--pa-accent-warning)]/10',   border: 'border-[var(--pa-accent-warning)]/30',   text: 'text-[var(--pa-accent-warning)]',   badge: 'Medium' },
  low:      { bg: 'bg-[var(--pa-accent-blue-soft)]',     border: 'border-[var(--pa-accent-blue)]/30',     text: 'text-[var(--pa-accent-blue)]',     badge: 'Low' },
  positive: { bg: 'bg-[var(--pa-accent-success-soft)]', border: 'border-[var(--pa-accent-success)]/30', text: 'text-[var(--pa-accent-success)]', badge: '🎉 Great' },
};
...
function SkCard() {
  return (
    <div className="pa-card space-y-3 p-5">
...
function SkChart({ h = 'h-52' }) {
  return (
    <div className={`pa-card p-5 ${h}`}>
      <Sk className="h-4 w-32 mb-4" />
      <div className="flex items-end gap-2 h-28">
        {[60, 85, 45, 95, 70, 55, 80].map((v, i) => (
          <div key={i} className="flex-1 bg-surface-secondary rounded-t-lg animate-pulse" style={{ height: `${v}%` }} />
        ))}
      </div>
    </div>
  );
...
function StatCard({ icon: Icon, label, value, sub, color = 'violet' }) {
  const map = {
    violet:  { bg: 'bg-accent-violet-soft',  ic: 'accent-violet',  val: 'accent-violet' },
    sky:     { bg: 'bg-[var(--pa-accent-blue-soft)]',     ic: 'text-[var(--pa-accent-blue)]',     val: 'text-[var(--pa-accent-blue)]' },
    emerald: { bg: 'bg-[var(--pa-accent-success-soft)]', ic: 'text-[var(--pa-accent-success)]', val: 'text-[var(--pa-accent-success)]' },
    amber:   { bg: 'bg-[var(--pa-accent-warning)]/10',   ic: 'text-[var(--pa-accent-warning)]',   val: 'text-[var(--pa-accent-warning)]' },
    rose:    { bg: 'bg-[var(--pa-accent-danger)]/10',    ic: 'text-[var(--pa-accent-danger)]',    val: 'text-[var(--pa-accent-danger)]' },
    indigo:  { bg: 'bg-accent-violet-soft',  ic: 'accent-violet',  val: 'accent-violet' },
  };
...
function Section({ title, icon: Icon, iconColor = 'accent-violet', children }) {
  return (
...
function Empty({ icon: Icon, message, sub }) {
  return <EmptyState icon={Icon} title={message} description={sub} />;
}
...
function CT({ active, payload, label, unit = '' }) {
  ...
  return (
    <div className="rounded-xl border border-subtle bg-surface px-3 py-2 shadow-md text-xs space-y-0.5">
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <svg width="130" height="130" viewBox="0 0 130 130">
        <circle cx="65" cy="65" r={r} fill="none" stroke="var(--pa-border)" strokeWidth="10" />
        <circle
          cx="65" cy="65" r={r}
          fill="none"
          stroke={color}
        ...
        />
        <text x="65" y="61" textAnchor="middle" fontSize="24" fontWeight="700" fill="var(--pa-text-primary)">{score}</text>
        <text x="65" y="77" textAnchor="middle" fontSize="10" fill="var(--pa-text-muted)">/ 100</text>
      </svg>
      <p className="text-sm font-semibold text-secondary">Productivity Score</p>
      <span
  ...
  return (
    <div>
      <div className="flex justify-between text-xs text-secondary mb-1">
        <span>{label}</span>
        <span className="font-semibold text-secondary">{value}{typeof max === 'number' && max !== 100 ? ` / ${max}` : '%'}</span>
      </div>
...
export default function Insights() {
  ...
  if (error) {
    return (
      <div className="flex items-center justify-between rounded-xl border border-[var(--pa-accent-danger)]/30 bg-[var(--pa-accent-danger)]/10 p-4 text-sm text-[var(--pa-accent-danger)]">
        ...
        <button type="button" onClick={load} className="pa-btn-secondary px-3 py-1.5 text-xs font-semibold">Retry</button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-10">
      ...
      <PageHeader
        title="Insights"
        description="A complete view of your productivity and learning progress."
      />
      ...
      {hasAnyData && (
        <>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-5">
            <div className="pa-card flex items-center justify-center p-6 lg:col-span-1">
          ...
          {/* ══ TASK ANALYTICS ═══════════════════════════════════════════════ */}
          {tasks.total > 0 && (
            <Section title="Task Analytics" icon={CheckCircle2} iconColor="text-[var(--pa-accent-success)]">
              ...
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                {/* 7-day line chart */}
                <div className="pa-card p-5">
                  <p className="mb-4 text-sm font-semibold text-secondary">7-Day Task Activity</p>
                  <ResponsiveContainer width="100%" height={180}>
                    <LineChart data={tasks.weeklyTrend} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="var(--pa-chart-grid)" />
                      <XAxis dataKey="day"       tick={{ fontSize: 11, fill: 'var(--pa-chart-tick)' }} />
                      <YAxis allowDecimals={false} tick={{ fontSize: 11, fill: 'var(--pa-chart-tick)' }} />
                      <Tooltip content={<CT />} />
                ...
                <div className="pa-card p-5">
                  <p className="mb-4 text-sm font-semibold text-secondary">Completed vs Pending</p>
                  {taskPie.length > 0 ? (
                    <ResponsiveContainer width="100%" height={180}>
                      <PieChart>
              ...
              {tasks.manual > 0 && tasks.generated > 0 && (
                <div className="pa-card p-5">
                  <p className="mb-3 text-sm font-semibold text-secondary">Task Source Breakdown</p>
                  <div className="flex gap-4">
                    {taskSourcePie.map((d, i) => (
                      <div key={d.name} className="flex items-center gap-2 text-sm text-secondary">
                        ...
                        {d.name}: <span className="font-semibold text-primary">{d.value}</span>
                      </div>
                    ))}
                  </div>
          ...
          {mockTests.totalAttempts > 0 && (
            <Section title="Mock Test Analytics" icon={ClipboardList} iconColor="accent-violet">
              ...
              {mockTests.trend.length > 1 && (
                <div className="pa-card p-5">
                  <p className="mb-4 text-sm font-semibold text-secondary">Score Progress Over Time</p>
                  <ResponsiveContainer width="100%" height={190}>
                    <AreaChart data={mockTests.trend} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                      ...
                      <CartesianGrid strokeDasharray="3 3" stroke="var(--pa-chart-grid)" />
                      <XAxis dataKey="date"    tick={{ fontSize: 10, fill: 'var(--pa-chart-tick)' }} />
                      <YAxis domain={[0, 100]} tick={{ fontSize: 11, fill: 'var(--pa-chart-tick)' }} />
              ...
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                ...
                {mockTests.subjectPerformance.length > 0 && (
                  <div className="pa-card p-5">
                    <p className="mb-4 text-sm font-semibold text-secondary">Average by Subject</p>
                    <ResponsiveContainer width="100%" height={180}>
                      <BarChart data={mockTests.subjectPerformance} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="var(--pa-chart-grid)" />
                        <XAxis dataKey="subject" tick={{ fontSize: 10, fill: 'var(--pa-chart-tick)' }} />
                        <YAxis domain={[0, 100]} tick={{ fontSize: 11, fill: 'var(--pa-chart-tick)' }} />
                        <Tooltip content={<CT unit="%" />} />
                ...
                {mockTests.difficultyPerformance.length > 0 && (
                  <div className="pa-card p-5">
                    <p className="mb-4 text-sm font-semibold text-secondary">Average by Difficulty</p>
                    <ResponsiveContainer width="100%" height={180}>
                      <BarChart data={mockTests.difficultyPerformance} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="var(--pa-chart-grid)" />
                        <XAxis dataKey="difficulty" tick={{ fontSize: 11, fill: 'var(--pa-chart-tick)' }} />
                        <YAxis domain={[0, 100]}     tick={{ fontSize: 11, fill: 'var(--pa-chart-tick)' }} />
                        <Tooltip content={<CT unit="%" />} />
          ...
          {/* ══ GOAL ANALYTICS ═══════════════════════════════════════════════ */}
          {goals.total > 0 && (
            <Section title="Goal Analytics" icon={Target} iconColor="text-[var(--pa-accent-warning)]">
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                {[
                  ...
                  <div key={label} className="pa-card p-4 text-center">
                    <p className="text-xl font-bold text-[var(--pa-accent-warning)]">{value}</p>
                    <p className="text-xs text-secondary">{label}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="pa-card p-5 space-y-4">
                <p className="text-sm font-semibold text-secondary">Goal Progress Overview</p>
                ...
                {goals.buckets.length > 0 && (
                  <div className="pt-2">
                    <p className="text-xs font-medium text-secondary mb-2">Distribution</p>
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                      {goals.buckets.map(({ label, value }) => (
                        <div key={label} className="rounded-xl border border-subtle bg-surface-secondary p-3 text-center">
          ...
          {/* ══ JOURNAL ANALYTICS ════════════════════════════════════════════ */}
          {journals.total > 0 && (
            <Section title="Journal Analytics" icon={BookOpen} iconColor="text-[var(--pa-accent-danger)]">
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                {[
                  ...
                  <div key={label} className="pa-card p-4 text-center">
                    <p className="text-xl font-bold text-[var(--pa-accent-danger)]">{value}</p>
                    <p className="text-xs text-secondary">{label}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                ...
                <div className="pa-card p-5">
                  <p className="mb-4 text-sm font-semibold text-secondary">Study Hours — Last 7 Days</p>
                  <ResponsiveContainer width="100%" height={180}>
                    <BarChart data={journals.weeklyStudyTrend} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="var(--pa-chart-grid)" />
                      <XAxis dataKey="day"   tick={{ fontSize: 11, fill: 'var(--pa-chart-tick)' }} />
                      <YAxis allowDecimals  tick={{ fontSize: 11, fill: 'var(--pa-chart-tick)' }} />
                ...
                {journals.moodDistribution.length > 0 && (
                  <div className="pa-card p-5">
                    <p className="mb-4 text-sm font-semibold text-secondary">Mood Distribution</p>
                    <div className="space-y-2.5">
                      {journals.moodDistribution.map(({ mood, count }) => {
                        const cfg = MOOD_LABEL[mood] || { emoji: '❓', label: mood, color: 'text-secondary bg-surface-secondary' };
                        ...
                        return (
                          <div key={mood} className="flex items-center gap-3">
                            ...
                            <div className="flex-1 h-2 overflow-hidden rounded-full bg-surface-secondary">
                              <div className="h-full rounded-full bg-[var(--pa-accent-danger)]/60 transition-all duration-700" style={{ width: `${pct}%` }} />
                            </div>
                            <span className="w-8 text-right text-xs text-secondary">{count}</span>
                          </div>
                        );
                      })}
                    </div>
                    {journals.avgStudyHours > 0 && (
                      <p className="mt-4 text-xs text-muted">
                        Avg <span className="font-semibold text-secondary">{journals.avgStudyHours}h</span> study per entry
                      </p>
                    )}
                  </div>
                )}
          ...
          {/* ══ NOTES SNAPSHOT ════════════════════════════════════════════════ */}
          {notes.total > 0 && (
            <Section title="Notes Snapshot" icon={StickyNote} iconColor="accent-violet">
              ...
              {notes.topTags.length > 0 && (
                <div className="pa-card p-5">
                  <p className="mb-3 text-sm font-semibold text-secondary">Top Tags</p>
                  <ChipRow items={notes.topTags.map(({ tag, count }) => ({
                    label: `#${tag}`,
                    count,
                    color: 'bg-accent-violet-soft accent-violet',
                  }))} />
                </div>
              )}
            </Section>
          ...
          {recommendations.length > 0 && (
            <Section title="Recommended Next Actions" icon={Lightbulb} iconColor="text-[var(--pa-accent-warning)]">
